...
- **カバレッジレポートのファイル識別（path normalization）** でハマりやすい。次の原則を守る：
  - レポート上のファイル識別キーは実装依存で、絶対パス・リポジトリ相対パス・モジュール名・コンパイル後の生成物パスなど、いずれかの形式になりうる。
  - 対象ファイルの判定は **キー全体一致ではなく末尾一致（suffix match）** を基本とする。利用するレポートのキー形式は Phase 3 で 1 度実測して確定する。
  - BEFORE / AFTER のカバレッジ差分は **同一の path 正規化ロジック** を通す。左右でロジックが異なると差分判定が壊れる。
